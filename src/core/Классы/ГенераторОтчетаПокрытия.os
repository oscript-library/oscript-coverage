#Использовать json
#Использовать logos

Перем ПутьККаталогуПроекта; // Строка, к каталогу проекта
Перем ПутьККаталогуИсходников; // Строка, к каталогу исходников
Перем ПутьКФайлуСтатистики; // Строка, путь к файлу статистики
Перем ПутьКРабочемуКаталогу; // Строка, путь к рабочему каталогу
Перем ДанныеСтатистикиПокрытия; // Соответствие
Перем ГенерируемыеОтчеты; // Структура

Перем ПутиФайловВОтносительные;
Перем Лог;
Перем ШаблонИмениФайлаСтатистики; // Строка, шаблон имени файла статистики для поиска файлов
Перем ПутиЗамены; // Соответствие

Перем ТаблицаСтатистикиПокрытия; // ТаблицаЗначение

// Устанавливает путь к файлу статистик
//
// Параметры:
//   ПутьКфайлу - Строка - Путь к файлу статистики (по умолчанию stat.json)
//
//  Возвращаемое значение:
//   Объект.ГенераторОтчетаПокрытия - экземпляр этого объекта ГенераторОтчетаПокрытия
//
Функция ФайлСтатистики(Знач ПутьКфайлу = "stat.json") Экспорт

	ПутьКФайлуСтатистики = ПутьКфайлу;

	Возврат ЭтотОбъект;

КонецФункции

// Устанавливает шаблон имени файла статистики
//
// Параметры:
//   ШаблонИмени - Строка - шаблон имени файла статистики (по умолчанию stat*.json)
//
//  Возвращаемое значение:
//   Объект.ГенераторОтчетаПокрытия - экземпляр этого объекта ГенераторОтчетаПокрытия
//
Функция ИмяФайлаСтатистики(Знач ШаблонИмени = "stat*.json") Экспорт

	ШаблонИмениФайлаСтатистики = ШаблонИмени;

	Возврат ЭтотОбъект;

КонецФункции

// Устанавливает выходной-рабочий каталог 
//
// Параметры:
//   ПутьККаталогу - Строка - Путь к рабочему каталогу
//
//  Возвращаемое значение:
//   Объект.ГенераторОтчетаПокрытия - экземпляр этого объекта ГенераторОтчетаПокрытия
//
Функция РабочийКаталог(Знач ПутьККаталогу) Экспорт

	ПутьКРабочемуКаталогу = ПутьККаталогу;

	Возврат ЭтотОбъект;

КонецФункции

// Устанавливает признак конвертации путей в относительные
//
//  Возвращаемое значение:
//   Объект.ГенераторОтчетаПокрытия - экземпляр этого объекта ГенераторОтчетаПокрытия
//
Функция ОтносительныеПути() Экспорт

	ПутиФайловВОтносительные = Истина;

	Возврат ЭтотОбъект;

КонецФункции

// Устанавливает путь к каталогу с исходниками проекта
//
// Параметры:
//   ПутьККаталогу - Строка - Путь к каталогу с исходниками
//
//  Возвращаемое значение:
//   Объект.ГенераторОтчетаПокрытия - экземпляр этого объекта ГенераторОтчетаПокрытия
//
Функция КаталогИсходников(Знач ПутьККаталогу) Экспорт

	ПутьККаталогуИсходников = ПутьККаталогу;

	Возврат ЭтотОбъект;

КонецФункции

// Устанавливает путь к каталогу проекта
//
// Параметры:
//   ПутьККаталогу - Строка - Путь к каталогу проекта
//
//  Возвращаемое значение:
//   Объект.ГенераторОтчетаПокрытия - экземпляр этого объекта ГенераторОтчетаПокрытия
//
Функция КаталогПроекта(Знач ПутьККаталогу) Экспорт

	ПутьККаталогуПроекта = ПутьККаталогу;

	Возврат ЭтотОбъект;

КонецФункции

// Добавляет путь к замене
//
// Параметры:
//   ЗаменяемыйПуть - Строка - путь к замене
//   НовыйПуть - Строка - новый путь
//
//  Возвращаемое значение:
//   Объект.ГенераторОтчетаПокрытия - экземпляр этого объекта ГенераторОтчетаПокрытия
//
Функция ЗаменитьПуть(Знач ЗаменяемыйПуть, Знач НовыйПуть) Экспорт

	ПутиЗамены.Вставить(ЗаменяемыйПуть, НовыйПуть);

	Возврат ЭтотОбъект;

КонецФункции

// Возвращает таблицу с данными о статистики покрытия
//
//  Возвращаемое значение:
//   ТаблицаЗначений - таблица с данными о статистики покрытия
//
Функция ПолучитьТаблицуСтатистикиПокрытия() Экспорт
	Возврат ТаблицаСтатистикиПокрытия.Скопировать();
КонецФункции

// Добавляет отчет в формате Clover для генерации
//
// Параметры:
//   ИмяПроекта - Строка - Имя проекта 
//   ИмяФайлаОтчета - Строка - Имя файла отчета (по умолчанию <clover.xml>)
//
//  Возвращаемое значение:
//   Объект.ГенераторОтчетаПокрытия - экземпляр этого объекта ГенераторОтчетаПокрытия
//
Функция Clover(Знач ИмяПроекта, Знач ИмяФайлаОтчета= "clover.xml") Экспорт

	ГенерируемыеОтчеты.Вставить("ОтчетClover", Новый Структура("ИмяФайла, ИмяПроекта", ИмяФайлаОтчета, ИмяПроекта));

	Возврат ЭтотОбъект;

КонецФункции

// Добавляет отчет в формате GenericCoverage для генерации
//
// Параметры:
//   ИмяФайлаОтчета - Строка - Имя файла отчета (по умолчанию <genericCoverage.xml>)
//
//  Возвращаемое значение:
//   Объект.ГенераторОтчетаПокрытия - экземпляр этого объекта ГенераторОтчетаПокрытия
//
Функция GenericCoverage(Знач ИмяФайлаОтчета = "genericCoverage.xml") Экспорт

	ГенерируемыеОтчеты.Вставить("ОтчетGenericCoverage", ИмяФайлаОтчета);

	Возврат ЭтотОбъект;

КонецФункции

// Добавляет отчет в формате Cobertura для генерации
//
// Параметры:
//   ИмяФайлаОтчета - Строка - Имя файла отчета (по умолчанию <coverage.xml>)
//
//  Возвращаемое значение:
//   Объект.ГенераторОтчетаПокрытия - экземпляр этого объекта ГенераторОтчетаПокрытия
//
Функция Cobertura(Знач ИмяФайлаОтчета= "coverage.xml") Экспорт

	ГенерируемыеОтчеты.Вставить("ОтчетCobertura", Новый Структура("ИмяФайла", ИмяФайлаОтчета));

	Возврат ЭтотОбъект;

КонецФункции

// Выполняет формирование отчетов покрытия
//
Процедура Сформировать() Экспорт

	НоваяТаблицаСтатистикиПокрытия();

	ПрочитатьДанныеСтатистики();

	ОбработатьТаблицуСтатистикиПокрытия();

	СформироватьОтчетClover();
	СформироватьОтчетGenericCoverage();
	СформироватьОтчетCobertura();

КонецПроцедуры

#Область Вспомогательные_процедуры_и_функции

Процедура ПрочитатьДанныеСтатистики()

	МассивФайловСтатистики = НайтиФайлы(ПутьКРабочемуКаталогу, ШаблонИмениФайлаСтатистики);

	Для Каждого ФайлСтатистики Из МассивФайловСтатистики Цикл

		ПрочитатьФайлСтатистики(ФайлСтатистики);

	КонецЦикла;

	Если ЗначениеЗаполнено(ПутьКФайлуСтатистики) Тогда
		
		ФайлСтатистики = Новый Файл(ПутьКФайлуСтатистики);
		
		Если ФайлСтатистики.Существует() Тогда
			ПрочитатьФайлСтатистики(ФайлСтатистики);
		КонецЕсли;
		
	КонецЕсли;

	ТаблицаСтатистикиПокрытия.Сортировать("ПутьКФайлу, ИмяМетода, СтрокаМодуля");

КонецПроцедуры

Функция ПрочитатьФайлСтатистики(Знач ФайлСтатистики)
	
	ЧтениеТекста = Новый ЧтениеТекста(ФайлСтатистики.ПолноеИмя, КодировкаТекста.UTF8);

	СтрокаJSON = ЧтениеТекста.Прочитать();
	ЧтениеТекста.Закрыть();

	Парсер = Новый ПарсерJSON();

	ДанныеФайлаСтатистики = Парсер.ПрочитатьJSON(СтрокаJSON);
	
	Для Каждого Файл Из ДанныеФайлаСтатистики Цикл

		ДанныеФайла = Файл.Значение;

		ПутьКФайлу = ДанныеФайла.Получить("#path");

		ВыполнитьЗаменуПутей(ПутьКФайлу);
		
		Если НЕ ЭтоФайлПроекта(ПутьКФайлу) Тогда
			Продолжить;
		КонецЕсли;

		ДобавитьВДанныеСтатистикиПокрытия(ПутьКФайлу, ДанныеФайла);

	КонецЦикла;

	СвернутьТаблицуСтатистикиПокрытия();

КонецФункции

Процедура ОбработатьТаблицуСтатистикиПокрытия()

	СтрокаЗамены = СтрШаблон("%1%2", ПутьККаталогуПроекта, ПолучитьРазделительПути());

	Для каждого СтрокаТаблицыСтатистики Из ТаблицаСтатистикиПокрытия Цикл
		
		Если ПутиФайловВОтносительные Тогда

			ПутьКФайлу = СтрокаТаблицыСтатистики.ПутьКФайлу;
			СтрокаТаблицыСтатистики.ПутьКФайлу = ОчиститьПутьКФайлуОтПути(ПутьКФайлу, СтрокаЗамены);

		КонецЕсли;
		
		СтрокаТаблицыСтатистики.Покрыто = СтрокаТаблицыСтатистики.КоличествоВызовов > 0;
		СтрокаТаблицыСтатистики.Покрыто = Не СтрокаТаблицыСтатистики.Покрыто;
	КонецЦикла;

КонецПроцедуры

Функция ОчиститьПутьКФайлуОтПути(Знач ПутьКФайлу, Знач СтрокаЗамены)
	
	Возврат СтрЗаменить(ПутьКФайлу, СтрокаЗамены, "");
	
КонецФункции

Процедура ВыполнитьЗаменуПутей(ПутьКФайлу)
	
	Для каждого ПутьЗамены Из ПутиЗамены Цикл
		
		СтрокаПоиска = ПутьЗамены.Ключ;
		СтрокаЗамены = ПутьЗамены.Значение;

		Если СтрНачинаетсяС(ПутьКФайлу, СтрокаПоиска) Тогда
			
			НачальнаяСтрока = ПутьКфайлу;

			ПутьКФайлу = СтрЗаменить(ПутьКФайлу, СтрокаПоиска, СтрокаЗамены);
			Лог.Отладка("Выполнена замена:
			| начальная строка <%1> 
			| подстрока замены <%2>
			| строка замещения <%3>
			| результат <%4> ", НачальнаяСтрока, СтрокаПоиска, СтрокаЗамены, ПутьКФайлу);

		КонецЕсли;
		
	КонецЦикла;

КонецПроцедуры

Процедура ДобавитьВДанныеСтатистикиПокрытия(Знач ПутьКФайлу, ДанныеФайла)
	
	ФайлПокрытия = Новый Файл(ПутьКФайлу);
	ИмяКласса = ФайлПокрытия.ИмяБезРасширения;

	Для каждого ДанныеМетодов Из ДанныеФайла Цикл
		
		ИмяМетода = ДанныеМетодов.Ключ;

		Если ИмяМетода = "#path" Тогда
			Продолжить;
		КонецЕсли;
	
		СтрокиМетода = ДанныеМетодов.Значение;

		Для каждого ДанныеПокрытияСтроки Из СтрокиМетода Цикл
		
			СтрокаМодуля = ДанныеПокрытияСтроки.Ключ;
			КоличествоВызовов = ДанныеПокрытияСтроки.Значение.Получить("count");
			ВремяВыполнения = ДанныеПокрытияСтроки.Значение.Получить("time");

			ДобавитьВТаблицуСтатистики(ПутьКфайлу,
										ИмяКласса,
										ИмяМетода,
										СтрокаМодуля,
										КоличествоВызовов,
										ВремяВыполнения);

		КонецЦикла;

	КонецЦикла;

КонецПроцедуры

Процедура НоваяТаблицаСтатистикиПокрытия()

	ТаблицаСтатистикиПокрытия = Новый ТаблицаЗначений;

	ТаблицаСтатистикиПокрытия.Колонки.Добавить("ПутьКФайлу");
	ТаблицаСтатистикиПокрытия.Колонки.Добавить("Пакет");
	ТаблицаСтатистикиПокрытия.Колонки.Добавить("ИмяКласса");
	ТаблицаСтатистикиПокрытия.Колонки.Добавить("ИмяМетода");
	ТаблицаСтатистикиПокрытия.Колонки.Добавить("СтрокаМодуля");
	ТаблицаСтатистикиПокрытия.Колонки.Добавить("КоличествоВызовов");
	ТаблицаСтатистикиПокрытия.Колонки.Добавить("Покрыто");
	ТаблицаСтатистикиПокрытия.Колонки.Добавить("НеПокрыто");
	ТаблицаСтатистикиПокрытия.Колонки.Добавить("ВремяВыполнения");
	
КонецПроцедуры

Процедура СвернутьТаблицуСтатистикиПокрытия()

	ТаблицаСтатистикиПокрытия.Свернуть("ПутьКФайлу, Пакет, ИмяКласса, ИмяМетода, СтрокаМодуля, Покрыто, НеПокрыто", "КоличествоВызовов, ВремяВыполнения");
	
КонецПроцедуры

Процедура ДобавитьВТаблицуСтатистики(Знач ПутьКФайлу,
									Знач ИмяКласса,
									Знач ИмяМетода,
									Знач СтрокаМодуля,
									Знач КоличествоВызовов,
									Знач ВремяВыполнения)

	НоваяСтрока = ТаблицаСтатистикиПокрытия.Добавить();
	НоваяСтрока.ПутьКФайлу = ПутьКФайлу;
	НоваяСтрока.Пакет = ОпределитьПакетПоПутиФайла(ПутьКФайлу);
	НоваяСтрока.ИмяКласса = ИмяКласса;
	НоваяСтрока.ИмяМетода = ИмяМетода;
	НоваяСтрока.СтрокаМодуля = Число(СтрокаМодуля);
	НоваяСтрока.КоличествоВызовов = Число(КоличествоВызовов);
	НоваяСтрока.ВремяВыполнения = Число(ВремяВыполнения);

КонецПроцедуры

Функция ОпределитьПакетПоПутиФайла(Знач ПутьКФайлу)
	
	ФайлПокрытия = Новый Файл(ПутьКФайлу);
	ПутьККаталогуФайла = ФайлПокрытия.Путь;

	СтрокаЗамены = СтрШаблон("%1%2", ПутьККаталогуИсходников, ПолучитьРазделительПути());

	ПутьККаталогуФайла = ОчиститьПутьКФайлуОтПути(ПутьККаталогуФайла, СтрокаЗамены);

	МассивДанныхПакета = СтрРазделить(ПутьККаталогуФайла, ПолучитьРазделительПути(), Ложь);

	МассивПутиКПакету = Новый Массив;

	Для каждого ЭлементПути Из МассивДанныхПакета Цикл
		
		Если ЭтоСлужебноеИмя(ЭлементПути) Тогда
			Продолжить;
		КонецЕсли;

		МассивПутиКПакету.Добавить(ЭлементПути);
	КонецЦикла;

	ИмяПакета = "main";
	Если МассивПутиКПакету.Количество() > 0  Тогда
		ИмяПакета = СтрСоединить(МассивПутиКПакету, ".");
	КонецЕсли;

	Возврат ИмяПакета;

КонецФункции

Функция ЭтоСлужебноеИмя(Знач ПроверяемоеИмя) 
	
	ПроверяемоеИмя = ВРег(ПроверяемоеИмя);
	Возврат ПроверяемоеИмя = ВРег("Классы")
			ИЛИ ПроверяемоеИмя = ВРег("Classes")
			ИЛИ ПроверяемоеИмя = ВРег("Модули")
			ИЛИ ПроверяемоеИмя = ВРег("Modules");

КонецФункции

Функция ЭтоФайлПроекта(Знач ПутьКФайлу)

	ФайлПодходит = СтрНачинаетсяС(ПутьКФайлу, ПутьККаталогуИсходников);

	Возврат ФайлПодходит;

КонецФункции

Процедура СформироватьОтчетClover()

	Если НЕ ГенерируемыеОтчеты.Свойство("ОтчетClover") Тогда
		Возврат;
	КонецЕсли;

	ИмяПроекта = ГенерируемыеОтчеты.ОтчетClover.ИмяПроекта;
	ИмяФайла = ГенерируемыеОтчеты.ОтчетClover.ИмяФайла;

	ПутьКФайлуПокрытия = ОбъединитьПути(ПутьКРабочемуКаталогу, ИмяФайла);

	ГенераторОтчета = Новый ГенераторОтчетаClover(ПутьККаталогуИсходников, ИмяПроекта);

	ГенераторОтчета.Сформировать(ТаблицаСтатистикиПокрытия, ПутьКФайлуПокрытия);

КонецПроцедуры

Процедура СформироватьОтчетCobertura()

	Если НЕ ГенерируемыеОтчеты.Свойство("ОтчетCobertura") Тогда
		Возврат;
	КонецЕсли;

	ИмяФайла = ГенерируемыеОтчеты.ОтчетCobertura.ИмяФайла;

	ПутьКФайлуПокрытия = ОбъединитьПути(ПутьКРабочемуКаталогу, ИмяФайла);

	ГенераторОтчета = Новый ГенераторОтчетаCobertura(ПутьККаталогуПроекта);

	ГенераторОтчета.Сформировать(ТаблицаСтатистикиПокрытия, ПутьКФайлуПокрытия);

КонецПроцедуры

Процедура СформироватьОтчетGenericCoverage()

	Если НЕ ГенерируемыеОтчеты.Свойство("ОтчетGenericCoverage") Тогда
		Возврат;
	КонецЕсли;

	ИмяФайла = ГенерируемыеОтчеты.ОтчетGenericCoverage;

	ПутьКФайлуПокрытия = ОбъединитьПути(ПутьКРабочемуКаталогу, ИмяФайла);

	ГенераторОтчета = Новый ГенераторОтчетаGenericCoverage();

	ГенераторОтчета.Сформировать(ТаблицаСтатистикиПокрытия, ПутьКФайлуПокрытия);

КонецПроцедуры

Процедура ПутьКФайлуСтатистикиПоУмолчанию()

	Если ФайлСтатистикиУстановлен() Тогда
		Возврат;
	КонецЕсли;

	ПутьКФайлуСтатистикиВРабочемКаталоге = ОбъединитьПути(ПутьКРабочемуКаталогу, "stat.json");
	ПутьКФайлуСтатистики = ПутьКФайлуСтатистикиВРабочемКаталоге;

	Если ФайлСтатистикиУстановлен() Тогда
		Возврат;
	КонецЕсли;

	ПутьКФайлуСтатистикиВКаталогеПроекта = ОбъединитьПути(ПутьККаталогуПроекта, "stat.json");

	ПутьКФайлуСтатистики = ПутьКФайлуСтатистикиВКаталогеПроекта;

	Если ФайлСтатистикиУстановлен() Тогда
		Возврат;
	КонецЕсли;

КонецПроцедуры

Функция ФайлСтатистикиУстановлен()

	Лог.Отладка("Путь к файлу статистики: <%1>", ПутьКФайлуСтатистики);
	Лог.Отладка("       файл статистики существует: <%1>", ФайлСуществует(ПутьКФайлуСтатистики));

	Возврат ЗначениеЗаполнено(ПутьКФайлуСтатистики)
			И ФайлСуществует(ПутьКФайлуСтатистики);

КонецФункции

Функция ФайлСуществует(Знач ПутьКФайлу)

	Возврат Новый Файл(ПутьКфайлу).Существует();

КонецФункции

Процедура ПриСозданииОбъекта()

	ТекущийКаталогПроекта = ТекущийКаталог();

	ПутьКРабочемуКаталогу = ОбъединитьПути(ТекущийКаталогПроекта, "coverage");

	КаталогПроекта(ТекущийКаталогПроекта);

	ФайлаКаталогаИсходников = Новый Файл(ОбъединитьПути(ТекущийКаталогПроекта, "src"));

	Если ФайлаКаталогаИсходников.Существует() Тогда
		КаталогИсходников(ФайлаКаталогаИсходников.ПолноеИмя);
	КонецЕсли;

	ПутиФайловВОтносительные = Ложь;

	ГенерируемыеОтчеты = Новый Структура;

	ПутиЗамены = Новый Соответствие;

	ПутьКФайлуСтатистикиПоУмолчанию();

КонецПроцедуры

#КонецОбласти

Лог = Логирование.ПолучитьЛог("oscript.lib.coverage");