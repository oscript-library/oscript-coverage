...
Функция Cobertura(Знач ИмяПроекта, Знач ИмяФайлаОтчета= "coverage.xml") Экспорт

	ГенерируемыеОтчеты.Вставить("ОтчетCobertura", Новый Структура("ИмяФайла, ИмяПроекта", ИмяФайлаОтчета, ИмяПроекта));
...
Процедура СформироватьОтчетCobertura()
	...
	ИмяПроекта = ГенерируемыеОтчеты.ОтчетCobertura.ИмяПроекта;
	ИмяФайла = ГенерируемыеОтчеты.ОтчетCobertura.ИмяФайла;

	ПутьКФайлуПокрытия = ОбъединитьПути(ПутьКРабочемуКаталогу, ИмяФайла);

	ГенераторОтчета = Новый ГенераторОтчетаCobertura(ПутьККаталогуПроекта, ИмяПроекта);
