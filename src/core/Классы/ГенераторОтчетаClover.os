...
Процедура СоздатьОтчетClover(Знач ПутьКСтат, Знач ПутьКОтчетуClover) Экспорт
	...
	Для Каждого Файл Из ДанныеПокрытия Цикл
		...
		ТекущаяЗаписьXML.ЗаписатьАтрибут("name", СтрЗаменить(ПутьКФайлу, ПутьКИсходникам, "/home/travis/build/khorevaa/oscript-coverage/src"));
		ТекущаяЗаписьXML.ЗаписатьАтрибут("path", "");
